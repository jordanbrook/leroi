...
def mask_invalid_data(
    radar, field, add_to=None, correlation_length=2000, min_field=0, min_area=10, return_smooth=False
):
    """
...
def dt_from_fn(fns):
    ...
    if four:
        dt = ["-".join((i[:4], i[4:6], i[6:])) + "T" + ":".join((j[:2], j[2:], "00")) for i, j in zip(dates, times)]
    else:
        dt = ["-".join((i[:4], i[4:6], i[6:])) + "T" + ":".join((j[:2], j[2:4], j[4:])) for i, j in zip(dates, times)]
    if len(dt) != len(fns):
        raise ValueError("Filename comprehension failed! Check instructions in dt_from_fn")
